...
## M8 - UI 정돈 + 버그 픽스 + 브랜딩 리뉴얼
Deliverables:
- 우측 시장 패널의 `도형 오브젝트`/`운영 로그`를 하단 패널 탭으로 이동
- 운영 로그 오류 원인 분석 및 수정(표시/수집/파싱 경로 안정화)
- Pine Editor 레이아웃/경고 문구 위치 버그 수정
- 도형 오브젝트 패널 기능 확장(삭제 + 앵커 정보 표시)
- 차트 상호작용 버그 픽스(추세선/레이/사각형 선택 후 차트 사라짐)
- 자석(Magnet) 기능 구현
- TradingView 아이콘/링크 제거
- TradingService 트레이드마크(로고) 적용
- 헤더 레이아웃 정리(차트 Venue 셀렉터를 종목명 옆으로 이동)

Phased plan:
- M8-1: 패널 재배치 + 헤더 레이아웃 정리 + TradingView 레거시 요소 제거
- M8-2: 도형 오브젝트 UX 확장(삭제/앵커 정보) + 선택 시 차트 소실 버그 픽스
- M8-3: Pine Editor 레이아웃/경고 위치 수정 + 운영 로그 오류 수정
- M8-4: 자석 기능 구현 + 트레이드마크 적용 + 회귀 안정화

Progress note (2026-03-01):
- M8-1 완료: 우측 시장 패널의 `도형 오브젝트`/`운영 로그`를 하단 탭으로 이동했고(`Pine Editor/전략 테스터/트레이딩 패널`과 동급 탭), 우측 패널에서는 해당 섹션 렌더링을 제거했다. 차트 헤더의 Venue 셀렉터는 종목명 라인 옆으로 재배치했으며, TradingView 레거시 링크/아이콘 렌더 경로는 제거/비노출 상태를 확인했다.
- M8-2 완료: 하단 `도형 오브젝트` 탭에서 각 오브젝트 행에 `삭제` 액션을 추가했고, 타입별 앵커 정보(가격/시간, 시작·끝 시간+가격, 노트 위치+텍스트 미리보기)를 노출하도록 확장했다. 잠금/표시 토글 및 행 선택 동작은 유지했고, 추세선/레이/사각형 선택/해제·이동 중 차트 오버레이가 비정상 좌표로 무너질 수 있던 렌더 경로를 좌표 정규화/클램프 가드로 보강해 차트 소실 재현 경로를 차단했다.
- M8-3 완료: Pine Editor에서 미저장 경고 문구를 툴바 상태 배지로 이동해 본문과 겹치지 않게 정리했고, 에디터 텍스트 영역을 `minmax(0, 1fr)` + 내부 스크롤 구조로 고정해 패널 경계 밖 overflow를 차단했다. 운영 로그는 feed 파싱 경로에 이벤트 정규화/폴백을 추가해 malformed payload(null/누락 필드/잘못된 타입)에서도 렌더 경로가 중단되지 않도록 안정화했다.
- M8-4 완료: 좌측 차트 컨트롤에 Magnet on/off 토글을 분리해 추가하고, on 상태에서 도형 생성/드래그 편집(추세선/레이/사각형/노트, 수평/수직선 이동 포함) 시 최근접 캔들 시간 버킷 + OHLC 가격 앵커로 결정론적 스냅이 적용되도록 반영했다. 상단 브랜드 영역에는 다크 테마 일관성을 유지하는 TradingService 인앱 트레이드마크(SVG)를 추가했고, 헤더/패널 레이아웃 및 기존 TradingView 레거시 제거 상태를 회귀 확인했다.
- M8 status: complete (M8-1, M8-2, M8-3, M8-4 complete).

Follow-up requests (2026-03-01, post-M8):
- M8-F1 완료: 추세선/레이/사각형 선택 시 간헐적 차트 소실 버그 재발 이슈 재수정 (사용자 재현 기준 해결)
- M8-F2 완료: Pine Editor 가시성 개선을 위해 하단 패널 기본 높이를 상향하고 반응형 구간별 기본 높이를 재조정함
- M8-F3 완료: 하단 패널 상단에 드래그 리사이저를 추가해 안전 최소/최대 범위 내에서 높이 조절이 가능하며, 선택 높이를 localStorage에 저장/복원하도록 반영함. Data Window 상태 문구는 헤더 우측으로 재배치해 `커서 데이터 없음` / `최신 캔들 표시` 텍스트 클리핑 없이 표시되도록 수정함.
- M8-F4 완료: 상단 도구의 `알림` 액션을 제거하고, 알림 접근 경로를 우측 패널 `알림` 탭 단일 진입으로 정리해 중복 이동 동선을 제거함
- M8-F5 완료: 추세선/레이/사각형 선택/차트 클릭 연속 동작에서 차트 재초기화 경로를 제거하고, 오버레이 투영·선택 가드를 보강해 소실 재현 경로를 차단함
- M8-F6 완료: 수평선 선택 후 차트 클릭 시 포인터 지점과 다른 가격에 생성되던 정렬 오차를 수정하고, 클릭 좌표/가격 변환 가드 및 자석 on/off 정합성을 보강함
- M8-F7 완료: 하단 `도형 오브젝트` 탭 헤더에 `전체 삭제` 액션을 추가하고, 전체 삭제 시 확인 대화상자를 거쳐 일괄 삭제되도록 반영함
- M8-F8 완료: 추세선/레이/사각형에 첫 클릭(anchor) 포인트를 오버레이로 표시하고, 도형 생성 중 1차 클릭 상태(`시작점 고정`)에서도 기준 포인트가 즉시 보이도록 보강함
- 진행 순서: M8-F1 → M8-F2 → M8-F3 → M8-F4 → M8-F5 → M8-F6 → M8-F7 → M8-F8 (follow-up sequence completed)

DoD:
- 우측 패널에서 옮긴 기능이 하단 패널 탭에서 동일/개선 UX로 동작
- 운영 로그에 남던 오류가 재현되지 않거나 원인/폴백이 명확히 처리됨
- Pine Editor 문구/편집창 overflow 문제 해결, 반응형에서도 레이아웃 안정
- 도형 오브젝트에서 삭제 가능 + 어떤 지점(시간/가격/타입) 기반인지 확인 가능
- 추세선/레이/사각형 선택 후 차트 소실 버그 재현 불가
- 자석 on/off와 스냅 동작이 도형 생성/이동 경로에서 일관되게 동작
- TradingView 링크/아이콘 제거 및 TradingService 트레이드마크 적용
- lint/build/test + 핵심 수동 스모크(UI 탭/도형/편집기/로그) 통과
