...
- M8-F1: 추세선/레이/사각형 선택 시 간헐적 차트 소실 버그 재발 이슈 재수정 (사용자 재현 기준 해결)
- M8-F2: Pine Editor 가시성 개선(하단 패널 기본 높이 상향)
- M8-F3: 하단 패널 높이 조절(드래그 리사이저) 기능 추가
- M8-F4: 상단 도구의 `알림` 액션 제거(우측 패널 이동 기능 중복 제거)
